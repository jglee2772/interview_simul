...
# ⚠️ 중요: 화면에 보이는 키를 정확히 복사하세요!
# 형식: test_sk_xxx (콜론 없이 단일 문자열, 코드에서 자동으로 "키:" 형식으로 인코딩)
# 테스트: test_sk_xxx 형태
# 실서비스: live_sk_xxx 형태 (사업자 등록 필요)
# 
# 이미지에서 확인한 키: test_sk_yZqmkKeP8gaG44gmNZ0BVbQRxB91
# 주의: 마지막 문자가 1(숫자)인지 l(소문자 L)인지 정확히 확인하세요!
TOSS_SECRET_KEY=test_sk_yZqmkKeP8gaG44gmNZ0BVbQRxB91

# ❌ 잘못된 예시:
# TOSS_SECRET_KEY=test_sk_yZqmkKeP8gaG44gmNZ0BVbQRxB91  (콜론 없음 - 오류 발생)
# TOSS_SECRET_KEY=test_sk_xxx:보안키  (보안 키와 혼동 - 잘못됨)
```

**시크릿 키 확인 방법:**
1. 토스페이먼츠 개발자센터 → API 개별 연동 키 섹션
2. "시크릿 키" 옆의 **"보기"** 버튼 클릭
3. **팝업 창이 열리면** 전체 키가 표시됩니다
4. **전체 키를 복사** (예: `test_sk_xxx:비밀번호부분` - 콜론 포함)
5. `.env` 파일에 그대로 붙여넣기

**⚠️ 주의:**
- 화면에 보이는 `test_sk_xxx`만 복사하면 안 됩니다
- "보기" 버튼을 눌러 **팝업에서 전체 키**를 복사해야 합니다
- 전체 키는 반드시 콜론(`:`)을 포함합니다
- 예: `test_sk_yZqmkKeP8gaG44gmNZ0BVbQRxB9l:비밀번호부분`

**참고:**
- **클라이언트 키**: 프론트엔드에서 사용 (이미 설정됨)
- **시크릿 키**: 백엔드에서 사용 (위에 설정)
- **보안 키**: 웹훅 검증 등에 사용 (현재는 필요 없음)
